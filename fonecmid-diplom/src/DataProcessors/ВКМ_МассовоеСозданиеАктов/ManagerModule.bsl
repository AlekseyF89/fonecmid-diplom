...
Функция СозданиеСпискаНаСервере(Дата) Экспорт
	...
	Пока Результат.Следующий() Цикл
		...
		//Если СоздатьОбъект Тогда
			...
			Иначе
				ОбновленнаяРеализация = ОбновлениеРеализации(Результат, КонецМесяца(Дата));
				РеализацииСтруктура.Вставить("Договор", Результат.Договор);
				РеализацииСтруктура.Вставить("Реализация", ОбновленнаяРеализация);
			КонецЕсли;
		//Иначе
		//	РеализацииСтруктура.Вставить("Договор", Результат.Договор);
...
Функция ОбновлениеРеализации(Результат, Дата)

	ПоискДок = Результат.Реализация.Ссылка.ПолучитьОбъект();
	ПоискДок.Дата = Дата;
	ПоискДок.Договор = Результат.Договор;
	ПоискДок.Контрагент = Результат.Контрагент;
	ПоискДок.Организация = Результат.Организация;
	ПоискДок.ВКМ_ВыполнитьАвтозаполнение();
	ПоискДок.Ответственный = Пользователи.ТекущийПользователь();
	
	Если ПоискДок.ПроверитьЗаполнение() Тогда

		Попытка  
			НачатьТранзакцию();
			ПоискДок.Записать(РежимЗаписиДокумента.Проведение, РежимПроведенияДокумента.Неоперативный);
