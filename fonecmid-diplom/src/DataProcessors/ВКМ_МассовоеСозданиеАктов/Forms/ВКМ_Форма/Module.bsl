...
Процедура ВКМ_СоздатьРеализацию(Команда)
	...
	ДлительнаяОперация = ВКМ_ВыполнениеНаСервере();
...
&НаСервере
Функция ВКМ_ВыполнениеНаСервере()
	
	Возврат ДлительныеОперации.ВыполнитьФункцию(УникальныйИдентификатор,"Обработки.ВКМ_МассовоеСозданиеАктов.СозданиеСпискаНаСервере", Объект.ВКМ_Период);
